...
from transformers import XLNetModel, XLNetConfig, BertConfig, BertModel, RobertaConfig,RobertaModel
from .transformer import TransformerEncoder,TransformerEncoderLayer,TransformerEncoderMOE,TransformerEncoderMOEPLUS
...
class CrossAttentionWithMOE(nn.Module):
    def __init__(self, hp, d_modal1,d_modal2, d_model, nhead, dim_feedforward, dropout, num_layers=6,num_experts=4,top_k=2):
        super(CrossAttentionWithMOE, self).__init__()
        self.hp = hp
        self.d_modal1 = d_modal1
        self.d_modal2= d_modal2
        self.num_heads = nhead
    ...
    def get_network(self, layers=-1):
        return TransformerEncoderMOE(embed_dim=self.d_model,  # 30
                                  num_heads=self.num_heads,
                                  layers=max(self.layers, layers),
                                  attn_dropout=self.attn_dropout,
                                  relu_dropout=self.relu_dropout,
    ...
    def forward(self, input_modal1,input_modal2,Tmask=None,Amask=None,Vmask=None,whether_add_position=True,compensation=None,compensation_modal=None): #这里下面都不用给counterfactual attention type
        """
        传入格式：(seq_len, batch_size,emb_size)
        t: torch.Size([50, 32, 768])
        a: torch.Size([134, 32, 5])
        ...
        if compensation is None:
            if self.hp.d_tin==self.d_modal1 and self.hp.d_ain==self.d_modal2:
                ...
                encoded = self.net(modal1, modal2, modal2, Vmask, Amask, whether_add_position=whether_add_position)
            elif self.d_modal1==self.hp.model_dim_cross and self.d_modal2 == self.hp.model_dim_cross:
                encoded = self.net(modal1, modal2, modal2, None, None, whether_add_position=None)
        else:
            if compensation_modal=="text" and self.hp.d_ain==self.d_modal2:
                encoded= self.net(compensation, modal2, modal2,None,Amask,whether_add_position=whether_add_position)
            elif compensation_modal=="text" and self.hp.d_vin==self.d_modal2:
